...
length(unique(GO.enrichment.summary$slim_term)) #39 unique go terms
...
length(unique(BP_slim$slim_term)) #29
...
upreg.DEG.plot <- enriched.up  %>%  
  ...
  theme(plot.title = element_text(size = 25, face = "bold", hjust = 0.95)) +
                   theme(panel.border = element_blank(), # Set border
                         ...
                         panel.grid.minor = element_blank(), #Set minor gridlines
                         axis.line = element_line(colour = "black"), #Set axes color
                         plot.background=element_blank());upreg.DEG.plot  #Set the plot background #set title attributes
