...
gcount_filt2annot = match(gcount_filt$gene_id, annot_GO$gene_id) #match genes in datExpr to genes in annotation file, note I removed the # before query in this file before loading
...
# Count unique genes based on gene_id
unique_genes <- unique(filtered_Pverr.annot$gene_id)
num_unique_genes <- length(unique_genes)

```

The annotation file now only contains genes that were detected in our dataset that have annotation information. This is 19821 out of the 27439 genes in our dataset. 
...
# Make ID vector 
IDvector <- filtered_Pverr.annot$gene_id
