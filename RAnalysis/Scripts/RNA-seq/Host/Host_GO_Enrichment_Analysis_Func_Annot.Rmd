...
BP.filt2.u <- BP.filt %>% slice_min(order_by = over_represented_pvalue, n = 30)

MF.filt2.u <- MF.filt %>% slice_min(order_by = over_represented_pvalue, n = 30)

#rejoin dataframes

enriched.up <- rbind(MF.filt2.u, BP.filt2.u, CC.filt)
...
```{r}
write.csv(up.KO.05, file = "RAnalysis/Output/RNA-seq/KEGG/KO.05.up.csv")
write.csv(down.KO.05, file = "RAnalysis/Output/RNA-seq/KEGG/KO.05.down.csv")
```
...
head(KOdf)
str(KOdf)
write.csv(KOdf, file = "RAnalysis/Output/RNA-seq/KEGG/KOenrichmentsummary.csv", row.names = FALSE)
```
