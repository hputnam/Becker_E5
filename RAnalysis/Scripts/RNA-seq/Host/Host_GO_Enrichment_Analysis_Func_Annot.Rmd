...
missing<-as.data.frame(gcount_filt[row_nas])
#print(missing)
```
...
```{r}
    ...
    GO.wall<-goseq(pwf, gene2cat=GO.terms, test.cats=c("GO:BP", "GO:MF", "GO:CC"), method="Wallenius", use_genes_without_cat=TRUE)
...
#all genes
DEG <- filtered_Pverr.annot[filtered_Pverr.annot$gene_id %in% DEG.res$gene_id, ]
dim(DEG) 
DEG_names <- as.vector(DEG$gene_id)
gene_vector <- as.integer(filtered_Pverr.annot$gene_id%in%DEG_names)
names(gene_vector) <- filtered_Pverr.annot$gene_id
...
# Make length vector 
lengthVector <- filtered_Pverr.annot$length

```

#Calculate Probability Weighting Function
```{r}
pwf<-nullp(gene_vector, bias.data=lengthVector) #weight vector by length of gene
pwf.up<-nullp(gene_vector.up, bias.data=lengthVector) #weight vector by length of gene for upreg
pwf.down<-nullp(gene_vector.down, bias.data=lengthVector) #weight vector by length of gene for downreg
```
...
# Find enriched GO terms, "selection-unbiased testing for category enrichment amongst differentially expressed (DE) genes for RNA-seq data"
GO.wall<-goseq(pwf, gene2cat=annot_GO, test.cats=c("GO:CC", "GO:BP", "GO:MF"), method="Wallenius", use_genes_without_cat=TRUE)
#adjust p-values 
...
enriched.GO.05 <-GO.wall$category[GO.wall$bh_adjust<.05]
