...
#input data
```{r}
f_gene_groups=c('RAnalysis/Data/RNA-seq/Host/dendogram_datasheet.csv') 
```

#output directory + base name for the output graph
```{r}
outidx=c('RAnalysis/Output/RNA-seq/Dendogram/Clusters 1a-gsea.graphs')
...
for(i in 1:length(f_gene_groups)){
  ...
  gg2 = gheatmap(gg1, scores2,offset=2.75, width=0.5,colnames_angle=-45,font.size=2,hjust=0) #,low='darkblue',high='steelblue1')
  #gg2 = gg2 + viridis::scale_fill_viridis(na.value=NA)
  gg2 = gg2 + viridis::scale_fill_viridis(na.value=NA, option = "inferno")
  pdf(paste0(outidx[i],'_tree_scores_.pdf'))
