...
```{r}
outidx=c('RAnalysis/Output/RNA-seq/Dendogram/DEG_clusters.BP.graphs')
...
for(i in 1:length(f_gene_groups)){
  ...
  gg1 = gg1 + geom_tippoint(aes(color = as.factor(cluster)), shape = "triangle", size = 6)
  gg1 = gg1 + labs(fill="% DEGs", color="Clusters") #changes legend names
  gg1 = gg1 + theme(legend.text=element_text(size=18, face = "bold")) #edits legend component labels font size
  gg1 = gg1 + theme(legend.title=element_text(size=20, face = "bold"))
  gg1 = gg1 + scale_fill_discrete(labels=c('anatomical structure development', 'signaling', 'transmembrane transport', 'vesicle-mediated transport', ''))
  gg1 = gg1 + scale_colour_manual(values =  colors1)
  gg1 = gg1 + geom_tiplab(offset=0.2, size=5.3) #font size for clusters,aes(color=as.factor(cluster)))
  ...
  gg2 = gheatmap(gg1, scores2, offset=4.5, width=0.1, font.size= 6) 
  gg2 = gg2 + viridis::scale_fill_viridis(na.value=NA, option = "inferno")
  pdf(paste0(outidx[i],'_tree_scores.pdf'),width=20, height = 40)
  print(gg2)
  dev.off()
