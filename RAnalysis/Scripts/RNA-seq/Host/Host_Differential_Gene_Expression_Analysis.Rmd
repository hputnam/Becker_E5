...
```{r}
length(unique(DEGSeq2.sig.results$gene_id)) # 252
down <- DEGSeq2.sig.results %>% filter(log2FoldChange < 0) 
length(unique(down$gene_id)) # DEGs downregulated 78
up <- DEGSeq2.sig.results %>% filter(log2FoldChange > 0)
length(unique(up$gene_id)) # DEGs upregulated 174

# Add a column indicating the direction of differential expression
DEGSeq2.sig.results <- DEGSeq2.sig.results %>%
  mutate(direction = ifelse(log2FoldChange < 0, "Downregulated", "Upregulated"))

# Sanity check to make sure all down and upregulated assigned properly
sum(DEGSeq2.sig.results$direction == "Downregulated") # DEGs downregulated 78
sum(DEGSeq2.sig.results$direction == "Upregulated") # DEGs upregulated 174
