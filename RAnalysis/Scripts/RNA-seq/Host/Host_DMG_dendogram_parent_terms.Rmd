...
#input data
```{r}
f_gene_groups=c('RAnalysis/Data/WGBS/dendogram/dendogram_datasheet_DMG.csv') 
```

#output directory + base name for the output graph
```{r}
outidx=c('RAnalysis/Output/WGBS/dendogram/DMG_clusters.graphs')
...
for(i in 1:length(f_gene_groups)){
  ...
  gg1 = gg1 + geom_tiplab(offset=0.2, size=6) #font size for clusters,aes(color=as.factor(cluster)))
  gg1 = gg1 + xlim(-3, 6) 
  ...
  gg2 = gheatmap(gg1, scores2, offset=5, width=0.1, font.size= 8) 
  gg2 = gg2 + viridis::scale_fill_viridis(na.value=NA, option = "inferno")
  pdf(paste0(outidx[i],'_tree_scores.pdf'),width=17, height = 22)
  print(gg2)
  dev.off()
