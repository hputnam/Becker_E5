...
#conclusion: All three files have the right number of gene_id rows = 27439.  
...
#Annot and KO appear to have an extra Pver_g1.t2 and are missing the word gene
Annot$gene_id <-  gsub("\\..*","", Annot$gene_id)
Annot$gene_id <-  gsub("Pver_g","Pver_gene_g",Annot$gene_id)
...
#Annot = Pver_gene_g162_gene_g164 while GFF3 = Pver_gene_g162_g164, so these rows are not joining
#Annot = Pver_gene_g408 GFF3 = Pver_split_gene_g408, so these rows are not joining
...
#format = Pver_novel_gene_281_5de57afd

split.GFF3  <- GFF3 %>%
  filter(str_detect(gene_id, "split"))
#format = Pver_split_gene_g408
