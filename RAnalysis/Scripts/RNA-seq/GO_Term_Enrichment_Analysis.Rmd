...
novel <- Genes %>%
  filter(str_detect(V9, "novel"))

split <- Genes %>%
  filter(str_detect(V9, "split"))

remain <- Genes %>%
  filter(!str_detect(V9, "novel")) %>% filter(!str_detect(V9, "split"))
...
length.struct.annot <- read.csv("RAnalysis/Genome/Pver_genome_assembly_v1.0.gene.gff3", header = FALSE, sep="", skip = 1)
...
# need to remove everything after g408 '-g408.t1-m18' for third format in KFS.KO file 

#bind all KO results
KO.terms <- left_join(Annot, KFS.KO)
KO.terms <- unique(KO.terms$ko)
str(KO.terms)
head(KO.terms)
...
# Obtain KO terms for DEGs to be used for the KEGG Mapper [(link)](https://www.genome.jp/kegg/mapper.html)
```{r}
ko <- filter(KO.terms, gene_id%in%gene_id) #Filter for gene IDs in cluster1
...
```{r}
#Perform goseq
KOwall <- goseq(pwf, GOref$gene_id, gene2cat=GO.terms, test.cats=c("KEGG"), method="Wallenius", use_genes_without_cat=TRUE)
