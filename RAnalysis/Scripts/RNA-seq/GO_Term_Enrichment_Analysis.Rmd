...
go.ref <- subset(annot, select= c(Gene)) #Select only relevant information
#rename Gene column to gene_id to match gcount data frame 
go.ref <- data.frame(gene_id=go.ref$Gene) #rename column
#filter gcount by available annotations
go.ref <- left_join(gcount, go.ref, by = "gene_id")
...
#length obtained from gff3 file
lengthVector <- length.struct.annot$length

#Calculate Probability Weighting Function
pwd.genes <- as.vector(DEG.res$gene_id)
pwd.genes=as.integer(go.ref$gene_id%in%pwd.genes)
names(pwd.genes)=go.ref$gene_id
length(pwd.genes)
length(names(pwd.genes))
length(unique(names(pwd.genes)))
sum(pwd.genes)

#uses gene bias 
pwf.C1<-nullp(DEgenes=pwd.genes, id=IDvector, bias.data=lengthVector) #weight vector by length of gene
...
BPplot <- GO.BP %>% filter(numInCat>5) %>% mutate(term = fct_reorder(term, -over_represented_pvalue)) %>%
...
MFplot <- GO.MF %>% filter(numInCat>5) %>% mutate(term = fct_reorder(term, -over_represented_pvalue)) %>% 
    ggplot(aes(x = ontology, y = term)) + 
    geom_tile(aes(fill=over_represented_pvalue, width = 1)) + 
      scale_y_discrete(position = "right") +
      facet_grid(slim_term~ ., scales = "free_y", labeller = label_wrap_gen(width = 10, multi_line = TRUE))+
...
ggsave("../../Output/RNA-seq/GOSeq/heatmap_GO_terms.pdf", fig5, width = 20, height = 20, units = c("in"))
...
write.csv(GO.enrichment.summary.table, '../../Output/RNA-seq/GOSeq/GO.enrichment.summary.table.ontology.csv') 
```
...
head(GOdf)
str(GOdf)
write.csv(GOdf, file = "../../Output/RNA-seq/GOSeq/GO.all.results.csv", row.names = FALSE)
```
