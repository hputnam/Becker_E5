...
GFF3$gene_id <-  gsub("Pver_gene_split_gene_g408","Pver_split_gene_g408",GFF3$gene_id)
...
nrow(novel.annot)+nrow(split.annot)+nrow(remain.annot) #should =27439

#Yes, we have the word novel and the word split it the Annot formats as well
...
#Annot has Pver_gene_g26291 in the gene_id column and Pver_split_gene_g26291-g26291.t1-m809 in the query column
#The word split occurs only inthe query, not the gene_id and needs to be replaced
novel.annot <- Annot %>%
  filter(str_detect(gene_id, "novel"))
nrow(novel.annot )

split.annot <- Annot %>%
  filter(str_detect(Query, "split"))
nrow(split.annot )
#These characters do not appear in gene_id for the GO file
...
split.annot$gene_id <-  gsub("Pver_gene_g","Pver_split_gene_g",split.annot$gene_id)

Annot <- rbind(remain.annot,novel.annot,split.annot)


test.GFF.GO <- full_join(GFF3, Annot, by = "gene_id")
nrow(test.GFF.GO)
#conclusion: we still have 84 more genes than the expected 27,439 after we merge these two files, a number of gene ID names are present in the GO annotation file in annot that do not exist the gene names of the GFF3
