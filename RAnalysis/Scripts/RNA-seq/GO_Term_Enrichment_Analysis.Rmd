...
colnames(GFF3) <- c("prot", "Predict", "id", "start","stop", "pos1", "pos2","pos3", "attr")
GFF3$length <- GFF3$stop - GFF3$start
GFF3$gene_id <-  GFF3$attr
GFF3$gene_id <-  gsub(";.*","",GFF3$gene_id) #remove extra characters
GFF3$gene_id <- gsub("ID=","",GFF3$gene_id) #remove extra characters
GFF3$gene_id <-  gsub("_gene","",GFF3$gene_id) #remove extra characters
head(GFF3)
...
KFS.KO <- KFS.KO[,c(1:2)]
Kegg.name <- as.data.frame(unique(KFS.KO$gene_id))
colnames(Kegg.name) <- c("gene_id")
