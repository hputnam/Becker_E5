...
Pver_annot <- read_csv("Functional_Annotation/Final_Annotations/pver_annot_full.csv")[-1]
Pver_annot$gene_id <-  gsub("\\..*","", Pver_annot$gene_id) #remove extra characters after the first .
#remove model after novel for novel genes to match gff3
Pver_annot$gene_id <-  gsub("Pver_novel_model_","Pver_novel_", Pver_annot$gene_id)
Pver_annot$gene_id <-  gsub("-.*","", Pver_annot$gene_id) #remove the extra g number after the dash
Pver_annot$gene_id <-  gsub("_gene_g","_g", Pver_annot$gene_id) #sub gene_g with _g
Pver_annot$gene_id <-  gsub("_gene_","_", Pver_annot$gene_id) #remove extra _

#load GFF3 and edit all naming information for gene_ID to match between data frames
#remove first column (x)
...
#check unmatched gene_id names
unmatched <- anti_join(GFF3, Pver_annot, by=c("gene_id"))

#remove last _g string after the first _g string
