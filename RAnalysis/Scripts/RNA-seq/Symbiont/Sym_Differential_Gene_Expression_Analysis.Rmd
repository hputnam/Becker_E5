...
## Follow RNAseq workflow in Bioinformatics>Becker_RNASeq_Workflow_C1.md before statistical analysis to make gene counts and transcript counts matrices
...
#gene count matrix
gcount <- as.data.frame(read.csv("RAnalysis/Data/RNA-seq/Symbiont/Sym_C1_gene_count_matrix.csv"))
...
heatmap
pdf(file = "RAnalysis/Output/RNA-seq/DEG/Symbiont/DESeq2_sample_heatmap_sym.pdf")
heatmap
dev.off()
...
ggsave(path = "RAnalysis/Output/Final_Figures/Symbiont/", filename = "DEGSeq2_general_PCA_plot_sym.pdf")
...
DESeq2_ma_plot_sym <- plotMA(DEGSeq2.results, ylim=c())
pdf(file = "RAnalysis/Output/RNA-seq/DEG/Symbiont/DESeq2_ma_plot_sym.pdf")
DESeq2_ma_plot_sym
dev.off()
...
DEGSeq2.sig.results.sym <- subset(DEGSeq2.results, padj < 0.05)
DEGSeq2.sig.results.sym <- as.data.frame(DEGSeq2.sig.results.sym)
DEGSeq2.sig.results.sym$gene_id <-rownames(DEGSeq2.sig.results.sym)
write.csv(DEGSeq2.sig.results.sym, 'RAnalysis/Output/RNA-seq/DEG/Symbiont/DEGSeq2.sig.results.sym.csv')  

```
## Identify unique genes from DEGs from treatments 
```{r}
dim(DEGSeq2.sig.results.sym) # 213 x 7
length(unique(DEGSeq2.sig.results.sym$gene_id)) # 213 unique genes between all treatments 
write.csv(DEGSeq2.sig.results.sym, file = "RAnalysis/Output/RNA-seq/DEG/Symbiont/DEGs.all_treatment_20230126_sym.csv")
```

## Visualize differentially expressed genes from DESeq2
```{r}
# Subset list of genes by those which padj>0.
DEGSeq2.output <- as.data.frame(DEGSeq2.sig.results.sym)
dim(DEGSeq2.output)
DEGSeq2.results.all <- DEGSeq2.sig.results.sym
...
pdf(file = "RAnalysis/Output/Final_Figures/Symbiont/DEGSeq2_DEG_heatmap_sym.pdf")
DEGSeq2_heatmap
dev.off()
...
ggsave(path = "RAnalysis/Output/Final_Figures/Symbiont/", filename = "DEGSeq2_DEG_PCA_plot_sym.pdf")

# PCA plot is of deferentially expressed genes only

# Save results

write.csv(counts(DEGSeq2_list), file="RAnalysis/Output/RNA-seq/DEG/Symbiont/DEGSeq2_list_unique_sym.csv")
...
```{r}
length(unique(DEGSeq2.sig.results.sym$gene_id)) # 213 unique DEGs
down <- DEGSeq2.sig.results.sym %>% filter(log2FoldChange < 0) 
length(unique(down$gene_id)) # 147 unique DEGs downregulated
up <- DEGSeq2.sig.results.sym %>% filter(log2FoldChange > 0)
