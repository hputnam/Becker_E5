...
## Follow RNAseq workflow in Bioinformatics>RNAseq workflow_C1 for symbionts before statistical analysis to make gene counts and transcript counts matrices
...
```{r}
#Set filter due to testing of number of genes and accuracy, also followed published literature and DESeq2 package recommendations 
filt <- filterfun(pOverA(0.9,5))
...
heatmap
pdf(file = "RAnalysis/Output/RNA-seq/DEG/Symbiont/sampletosample_heatmap_sym.pdf")
heatmap
dev.off()
...
```{r, message = FALSE}
DEGSeq2.results <- results(DEGSeq2_wald, contrast=c("treatment","control","enriched"), pAdjustMethod="fdr")#run wald model

DEGSeq2.results$padj <- p.adjust(DEGSeq2.results$pvalue, method="BH")
...
pdf(file = "RAnalysis/Output/RNA-seq/DEG/Symbiont/ma_plot_sym.pdf")
MA_PLOT
dev.off()
...
write.csv(DEGSeq2.sig.results, 'RAnalysis/Output/RNA-seq/DEG/Symbiont/DEGSeq2.sig.results.sym.csv')  
...
write.csv(DEGSeq2.sig.results, file = "RAnalysis/Output/RNA-seq/DEG/Symbiont/DEGs.all_treatment_20230126_sym.csv")
```
