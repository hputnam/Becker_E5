...
```{r, message = FALSE}
DEG_wald <- DESeq(gdds) #run differential expression test by group using the Wald model
```

Explore significant p-values for treatment
```{r, message = FALSE}
DEG.results <- results(DEG_wald, contrast=c("treatment","control","enriched")) #see code underlying function is to apply to glmer DEG results, FDR correction
head(DEG.results)
...
pdf(file = "../../Output/Final_Figures/DEG_heatmap.pdf")
DEG_heatmap
dev.off()
...
# For data with random effects, using a glmer glmm approach
...
#### Fit glmer position model, approaching with a negative binomial distribution approach
...
for(i in 1:length((gs))){
  ...
  # glmer negative binomial fit
  
  fit <- glmmadmb(count ~ treatment + (1|block), data=combined.data1, family="nbinom", debug=TRUE, save.dir = "tmp")
...
GLMER.results.all$padj <- p.adjust(GLMER.results.all$pval.treatment, method = "bonferroni", n = length(GLMER.results.all$pval.treatment))

#How many significant p-values were less than 0.05 after p adjust?

GLMER.sig.results.sum <- sum(GLMER.results.all$padj < 0.05, na.rm=TRUE) 

#create data frame with only DEG's with < 0.05 significance 

GLMER.sig.results <- subset(GLMER.results.all, padj < 0.05)

write.csv(GLMER.sig.results, '../../Output/RNA-seq/DEG/GLMER.sig.results.csv')  

read.csv('../../Output/RNA-seq/DEG/GLMER.sig.results.csv')  
