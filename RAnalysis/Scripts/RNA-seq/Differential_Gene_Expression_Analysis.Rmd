...
DEG.results.all <- subset(results, pval.treatment < 0.05)
...
DEG.results.all$padj <- p.adjust(DEG.results.all$pval.treatment, method = p.adjust.methods, n = length(DEG.results.all$pval.treatment))
...
dim(DEG.results.all)
DEGs <- DEG.results.all$gene_id # list all gene names 
...
DEGheatmap <- pheatmap(mat, scale= "row", legend=TRUE, annotation_legend=TRUE, annotation_col=df_DEG, annotation_colors = ann_colors,
...
ggsave(path = "../../Output/Final_Figures/", filename = "DEGheatmap.pdf")
...
DEGPCAplot <- ggplot(DEGPCAdata, aes(PC1, PC2, color=treatment)) +
  geom_point(size=3) +
...
ggsave(path = "../../Output/Final_Figures/", filename = "DEGPCAplot.pdf")
