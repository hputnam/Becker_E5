...
heatmap
pdf(file = "RAnalysis/Output/RNA-seq/DEG/SAMPLETOSAMPLE_HEATMAP.pdf")
heatmap
dev.off()
...
ggsave(path = "RAnalysis/Output/Final_Figures/", filename = "DEGSeq2_general_PCA_plot.pdf")
...
pdf(file = "RAnalysis/Output/RNA-seq/DEG/MA_PLOT.pdf")
MA_PLOT
dev.off()
...
write.csv(DEGSeq2.sig.results, 'RAnalysis/Output/RNA-seq/DEG/DEGSeq2.sig.results.csv')  
...
write.csv(DEGSeq2.sig.results, file = "RAnalysis/Output/RNA-seq/DEG/DEGs.all_treatment_20220616.csv")
```
...
pdf(file = "RAnalysis/Output/Final_Figures/DEGSeq2_DEG_heatmap.pdf")
DEGSeq2_heatmap
dev.off()
...
ggsave(path = "RAnalysis/Output/Final_Figures/", filename = "DEGSeq2_DEG_PCA_plot.pdf")

# PCA plot is of deferentially expressed genes only

# Save results

write.csv(counts(DEGSeq2_list), file="RAnalysis/Output/RNA-seq/DEG/DEGSeq2_list_unique.csv")
...
poc_down <- DEGSeq2.sig.results %>% filter(log2FoldChange < 0) 
length(unique(poc_down$gene_id)) # 147 unique DEGs downregulated
poc_up <- DEGSeq2.sig.results %>% filter(log2FoldChange > 0)
length(unique(poc_up$gene_id)) # 66 unique DEGs upregulated
```
