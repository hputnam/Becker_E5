...
treatmentinfo <- read.csv("RAnalysisData/RNA-seq/metadata.RNAseq.csv", header = TRUE, sep = ",")
rownames(treatmentinfo) <- treatmentinfo$sample_id
str(treatmentinfo)
head(treatmentinfo)

#gene count matrix
gcount <- as.data.frame(read.csv("RAnalysisData/RNA-seq/Poc_gene_count_matrix.csv", row.names="gene_id"))
dim(gcount)
head(gcount)
...
ggsave(path = "RAnalysisOutput/Final_Figures/", filename = "povera_ggplot.pdf")
...
```{r, message = FALSE}
DEGSeq2.results <- results(DEGSeq2_wald, contrast=c("treatment","control","enriched")) #see code underlying function is to apply to glmer DEG results, FDR correction
...
write.csv(DEGSeq2.sig.results, 'RAnalysisOutput/RNA-seq/DEG/DEGSeq2.sig.results.csv')  
...
pdf(file = "RAnalysisOutput/Final_Figures/DEGSeq2_heatmap.pdf")
DEGSeq2_heatmap
dev.off()
...
ggsave(path = "RAnalysisOutput/Final_Figures/", filename = "DEGSeq2_PCA_plot.pdf")

# PCA plot is of deferentially expressed genes only

# Save results

write.csv(counts(DEGSeq2_list), file="RAnalysisOutput/RNA-seq/DEG/DEGSeq2_list_unique.csv")
...
write.csv(GLMER.sig.results, 'RAnalysisOutput/RNA-seq/DEG/GLMER.sig.results.csv')  

read.csv('RAnalysisOutput/RNA-seq/DEG/GLMER.sig.results.csv')  
...
mixed.model.results <- GLMER.sig.results
