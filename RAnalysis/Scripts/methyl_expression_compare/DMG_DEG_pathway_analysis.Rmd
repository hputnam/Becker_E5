...
#filter for bh_adjust values
    DMG.parent.terms <- DMG %>%
...
#Reduce/collapse GO term set with the rrvgo package 
      simMatrix <- calculateSimMatrix(DMG.parent.terms$GOterm,
                                orgdb="org.Ce.eg.db", #c. elegans database
                                ont=category,
                                method="Rel")
      
#calculate similarity 
    scores <- setNames(-log(DMG.parent.terms$bh_adjust), DMG.parent.terms$GOterm)
    ...
    DMG.parent.terms<-DMG.parent.terms%>%
      filter(GOterm %in% reducedTerms$go)
    
    #add in parent terms to list of go terms 
    DMG.parent.terms$ParentTerm<-reducedTerms$parentTerm[match(DMG.parent.terms$GOterm, reducedTerms$go)]
    #DMG<-DMG%>%
  ...
  GO.plot <-  ggplot(DMG.parent.terms, aes(x = ontology, y = term)) + 
