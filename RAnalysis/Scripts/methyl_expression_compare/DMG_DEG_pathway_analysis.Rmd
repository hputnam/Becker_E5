...
# Make full gene and meth datframe and plot correlation for all genes
```{r}
## Mean Methylation vs Gene Expression following Liew et al
meth.express.all <- meth.express

meth.express.all  %>% drop_na()

#control log gene expression x % methylation
meth.express.all$log_gene <- log(meth.express.all$gene_mean_express+1)
meth.express.all$per_meth <- meth.express.all$mean_meth * 100

#make plot with correlation tests
#decide between pearson, kendall, or spearman
#first test normality
shapiro.test(meth.express.all$gene_mean_express) # <0.05
shapiro.test(meth.express.all$mean_meth) # <0.05
...
meth.express.all <- ggplot(meth.express.all,aes(x=per_meth, y=log_gene,)) + 
    geom_point() + 
  ...
  stat_cor(method = "kendall", digits = 3, label.x = 60, label.y = 0.5 ) +
    theme_cowplot();meth.express.all

### Figure Mean Methylation vs Gene Expression###

ggsave(meth.express.all, filename="RAnalysis/Output/meth_express_compare/global.express.meth.png", height = 7, width = 10)
```
