...
meth <- as.matrix(meth)
meth <- meth/100
saveRDS(m, "RAnalysis/Data/GE_meth_compare/meth_dataset.RData")
...
meth_summary <- cbind(mean_meth,mean_meth_E,mean_meth_C,cv_meth_E,cv_meth_C,cv_mean_AmongTrt_meth,
                      diff_Trt) # bind all data frames with summary stats
...
mean_express_E <- rowMeans(subset_express_E)

# Mean gene expression control
subset_C <- expression[, colnames(expression)%in%list_C] #subset data from main m matrix for just control data

mean_C <- rowMeans(subset_C)

# Coefficient of Variation for enriched gene expression
sd_E <- rowSds(subset_E)
cv_E <- sd_E/mean_E

# Coefficient of Variation for control gene expression
sd_C <- rowSds(subset_C)
cv_C <- sd_C/mean_C
