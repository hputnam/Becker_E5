...
df$density <- get_density(log(df$gene_cv_express_C+0.01), df$mean_meth_C*100, n = 100)
meanGbMvsGEcv_control <- ggplot(df,aes(log(x=gene_cv_express_C+0.01),y=mean_meth_C*100)) + 
    geom_point(aes(color = density)) + ylim(0,100) + scale_color_viridis() +
    geom_smooth(method=lm,colour="orange",size=2) +
    labs(x=bquote("Gene Expression"~CV[Ind]~"(log)"),y="DNA Methylation (%)") +
    theme_cowplot()
