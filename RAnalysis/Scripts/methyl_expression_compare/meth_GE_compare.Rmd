...
mean_E <- m[, colnames(m)%in%list_E] #subset data from main m matrix for just enriched data

#mean_E <- rowMeans(subset_E)

# Mean gene meth control
mean_C <- m[, colnames(m)%in%list_C] #subset data from main m matrix for just control data

#mean_C <- rowMeans(subset_C)

# Coefficient of Variation for enriched samples
sd_E <- rowSds(m[, list_E$treatment == "enriched"])
cv_E <- sd_E/mean_E
