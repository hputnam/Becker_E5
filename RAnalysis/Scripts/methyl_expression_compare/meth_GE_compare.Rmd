...
#make mean expression between treatments barplot
```{r}
...
express.E <- df %>% select(gene_id, gene_mean_express_E) 
express.E$treatment <- "enriched" #make column with treatment
names(express.E)[2] ="expression" #rename to expression

express.C <- df %>% select(gene_id, gene_mean_express_C) 
express.C$treatment <- "control" #make column with treatment
names(express.C)[2] ="expression" #rename to expression 
 
#combine to data frames to make plot
express <- rbind(express.C, express.E)

#calculate mean and std errors
means.express <- aggregate(expression ~ treatment, data=express, FUN=mean)
ses.express <- aggregate(expression ~ treatment, data=express, FUN=std.error)
means.express$ses <- ses.express$expression
means.express$expression <- means.express$expression

#make bar plot of mean expression between treatment
plot.express <- ggplot(express, aes(x=treatment, y=expression, fill = treatment)) +
  geom_bar(stat="identity",
    ...
    ylab("Mean Expression") + #plot y axis label
    ...
    theme_bw();plot.express

### Figure Mean Methylation vs Gene Expression###
ggsave(plot.express, filename="RAnalysis/Output/Final_Figures/Host/express.treatment.png", height = 7, width = 10)

# build the linear model to see if tretaments differ and check assumptions
model  <- lm(expression ~ treatment, data = express)
...
express.mod <- kruskal.test(expression ~ treatment, data = express)
