...
#input data
```{r}
f_gene_groups=c('RAnalysis/Data/RNA-seq/P.verrucosa/Host/dendogram/dendogram_datasheet_DEG_parent_terms.csv') 
```
...
for(i in 1:length(f_gene_groups)){
  ...
  gg1 = ggtree(hc,layout='rectangular') %<+% cutr_tightClusters #changes dendrogram layout/structure
  gg1 = gg1 + geom_tippoint(aes(color = as.factor(cluster)), shape = "triangle", size = 2) #edits size and shape of nodes
  gg1 = gg1 + scale_colour_manual(values =  colors1) #references colors of clusters to add to plot
  gg1 = gg1 + geom_tiplab(offset=0.1, size=2.75) #edits where names are set
  gg1 = gg1 + xlim(-3, 3) #changes plot width
  pdf(paste0(outidx[i],'_tree_parent_terms.pdf'))
  print(gg1)
  dev.off()
  ...
  pdf(paste0(outidx[i],'_tree_scores_parent_terms.pdf'),width=17, height = 22)
  print(gg2)
  dev.off() 
...
clusters <- read_csv("RAnalysis/Data/RNA-seq/P.verrucosa/Host/dendogram/DEG_dotplot.csv", col_names = TRUE)


gg3 = ggplot (clusters, aes(x = comparison, y = reorder(term_name,cluster), size = set_size)) + 
            geom_count (aes(color = percentDEInCat)) + 
            scale_color_viridis(na.value=NA, option = "inferno") +
            #scale_colour_gradient(low = "#FFFF66", high = "#CC0033", na.value = NA) +
            theme(panel.background = element_blank())

print(gg3)

ggsave("RAnalysis/Output/Dendograms/clusters_dotplot.png",
       dpi = 300, 
       width = 9,
       height = 15)
