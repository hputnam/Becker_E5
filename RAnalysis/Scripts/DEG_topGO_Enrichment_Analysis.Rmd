...
plot_data_BP <- filtered_direction_go_results_BP %>%
  group_by(GO.ID, direction, ParentTerm, Term) %>%
  summarise(numInCat = sum(numInCat), numDEI = sum(numDEI)) %>%
  ungroup() %>%
  mutate(percentDEGs = (numDEI / numInCat) * 100)

# Add Term labels to dendrogram data
dendro_data_BP$labels <- dendro_data_BP$labels %>%
  left_join(filtered_direction_go_results_BP %>% dplyr::select(GO.ID, Term, direction) %>% distinct(), by = c("label" = "GO.ID")) %>%
  mutate(label = Term)
...
dendo.BP.plot <- ggplot() +
  ...
  geom_text(data = dendro_data_BP$labels,
            aes(x = x, y = y, label = label, hjust = 1, color = direction),
            size = 8, nudge_y = -0.01) +
  ...
  geom_point(data = plot_data_BP,
             aes(x = match(Term, dendro_data_BP$labels$label),
                 y = -2.2,
                 size = numInCat,
                 fill = percentDEGs),  # Use fill for color gradient
  ...
  scale_color_manual(values = c("Up" = "red", "Down" = "blue"),
                     ...
                     name = "Gene Expression Direction",  # Change legend title here
                     guide = guide_legend(override.aes = list(size = 8))) +
  scale_fill_gradient(low = "coral", high = "purple3", 
                      name = "% DEGs", 
                      limits = c(0, 6),  # Set limits based on your data range
                      breaks = seq(0, 6, by = 3)) +  # Set custom breaks
  coord_flip() +
  ylim(-2.2, NA) +  # Extend the x-axis (effectively the y-axis) to -1.4
  ...
  labs(x = NULL, y = "Biological Processes", size = "Number of Genes", fill = "Percent DEGs");dendo.BP.plot

# Save the updated plot
ggsave("RAnalysis/Output/DEG_dendrogram_plot_BP.png", 
       plot = dendo.BP.plot, 
       width = 15,  
       height = 14, 
       units = "in", 
...
#Prepare data for plotting by grouping by GO.ID, direction, ParentTerm, and Term
plot_data_MF <- filtered_direction_go_results_MF %>%
  group_by(GO.ID, direction, ParentTerm, Term) %>%
  summarise(numInCat = sum(numInCat), numDEI = sum(numDEI)) %>%
  ungroup() %>%
  mutate(percentDEGs = (numDEI / numInCat) * 100)
...
dendro_data_MF$labels <- dendro_data_MF$labels %>%
  left_join(direction_go_results_MF %>% dplyr::select(GO.ID, Term, direction) %>% distinct(), by = c("label" = "GO.ID")) %>%
  mutate(label = Term)
...
dendo.MF.plot <- ggplot() +
  ...
  scale_fill_gradient(low = "coral", high = "purple3", 
                      name = "% DEGs", 
                      limits = c(0, 15),  # Set limits based on your data range
                      breaks = seq(0, 15, by = 5)) +  # Set custom breaks
  coord_flip() +
  ylim(-2.4, NA) +  # Extend the x-axis (effectively the y-axis) to -1.4
  ...
  theme(axis.ticks.y = element_blank(),
        ...
        axis.title.x = element_text(size = 30, face = "bold")) + # Increase legend title size
  # Add labels for axes and legends
  labs(x = NULL, y = "Molecular Functions", size = "Number of Genes", fill = "Percent DEGs");dendo.MF.plot

# Save the updated plot
ggsave("RAnalysis/Output/DEG_DEG_DEG_dendrogram_plot_MF.png", 
       plot = dendo.MF.plot, 
       width = 15,  
       height = 14, 
       units = "in", 
...
ggsave("RAnalysis/Output/DEG_DEG_DEG_dendrogram_plot_CC.png", plot = dendo.CC.plot, width = 11, height = 8, units = "in")
