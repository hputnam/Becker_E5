...
# Filter the dataset based on numDEI cut off of at least two sig terms and terms to remove
filtered_direction_go_results_BP <- direction_go_results_BP %>%
  filter(!ParentTerm %in% terms_to_remove, numDEI >= 2)
...
# Combine similarity matrices for both directions
combined_simMatrix_MF <- calculateSimMatrix(direction_go_results_MF$GO.ID,
                                         orgdb = "org.Ce.eg.db",
                                         ont = "MF",
                                         method = "Rel")
