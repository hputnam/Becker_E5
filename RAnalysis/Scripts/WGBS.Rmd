...
# Filter 'MD.10x' for genes in 'DMG.sig.annot.10x'
methylation_data <- MD.10x %>%
  filter(gene %in% DMG.sig.annot.10x$gene)

# Get unique genes
genes <- unique(methylation_data$gene)

# Initialize a list to store results
results <- list()

# Run t-test for each gene to calculate hyper and hypo methylation
for (gene in genes) {
  # Subset data for the current gene
  gene_data <- methylation_data %>% filter(gene == !!gene)
  
  # Run t-test
  t_test_result <- tryCatch({
    t.test(per.meth ~ treatment, data = gene_data)
  }, error = function(e) {
    return(NULL)
  })
  ...
  if (!is.null(t_test_result)) {
    ...
    enriched_mean <- mean(gene_data$per.meth[gene_data$treatment == "enriched"])
    control_mean <- mean(gene_data$per.meth[gene_data$treatment == "control"])
...
# Classify genes in hyper or hypo methylated condition
result_df <- result_df %>%
  mutate(
    methylation_status = case_when(
      enriched_mean > control_mean ~ "hypermethylated",
      enriched_mean < control_mean ~ "hypomethylated",
      TRUE ~ "no_significant_difference"
    )
  )

# Count genes in each category
methylation_counts <- result_df %>%
