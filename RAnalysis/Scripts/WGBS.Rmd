...
sub_meth_table.10x  <- MD.10x %>% 
  ...
  #filtering out loci that have a median methylation of <5% across all samples
   group_by(Loc) %>%
   mutate(loci_median = median(per.meth)) %>%
   filter(loci_median >= 5) %>% ungroup() %>%
...
## After filtering for 5 **methylated** positions filtering 150,916 rows for 342 genes (length of gs.10x above)
```
...
run_glm <- function(data){
  ...
  write.table(results.10x$Stats[Gene, ], "RAnalysis/Output/GLM_output_with_extrafilter.txt", 
...
```{r}
glm_results <- read.delim2("RAnalysis/Output/GLM_output_with_extrafilter.txt", header=F, sep = " ") %>% 
...
## With filtering for 5 methylated positions, 206 genes are significantly differentially methylated 
```
...
Assigning 'methylation pattern's based on the change in mean methylation above 
```{r}
relative_change <- relative_change %>%
  mutate(pattern_group = ifelse(relative_change < 0, 'hypomethylated', 'hypermethylated'))
```
...
pcoa_coordinates %>%
  ...
  annotate("text", x = -100, y = -97, label = "PERMANOVA p=0.01", 
           hjust = 0, #vjust = 1.1, 
           size = 3, color = "grey20");pcoa_coordinates

ggsave("RAnalysis/Output/DMG_PCoA.png", width = 6.5, height = 5)
...
count_hypermethylated <- sum(final_df$methylation_status == 'Hypermethylated')
count_hypomethylated <- sum(final_df$methylation_status == 'Hypomethylated')
...
DMG.volcano <- ggplot(final_df, aes(x = log2FoldChange, y = -log10(padj), color = methylation_status)) +
  ...
  scale_color_manual(values = colors, name = "Methylation Status", 
                     ...
                     labels = c("Hypermethylated (88)", "Hypomethylated (118)", "Not Significant"),
...
DMG.PCoA <- readPNG("RAnalysis/Output/DMG_PCoA.png")
PCoA.DMG <- rasterGrob(DMG.PCoA, interpolate=TRUE)

Figure2 <- plot_grid( DMG.volcano, PCoA.DMG, labels = c('A', 'B'), label_size = 14)

ggsave(filename=paste0("RAnalysis/Output/DMG.volcano_pcoa.png"), plot=Figure2, dpi=300, width=10, height=5, units="in", limitsize = FALSE)
