...
#all code below is commented out because only need to run this for the initial step, can just load Rdata below to move to next sections, last ran 20240809
...
num_hyper_genes <- length(hyper_genes) #195 Count of hypermethylated genes
...
DMG.volcano <- ggplot(final_df, aes(x = log2FoldChange, y = -log10(padj), color = methylation_status)) +
  ...
  scale_color_manual(values = colors, name = "Methylation Status", 
                     ...
                     labels = c("Hypermethylated (195)", "Hypomethylated (158)", "Not Significant"),
...
pca_result <- prcomp(test2[1:353], center = TRUE, scale. = TRUE)

# Visualize PCA eigenvalues
fviz_eig(pca_result)

# scale data
scaled_test2 <- scale(test2[1:353])
