...
# Take all filtered sub_meth_table_10x file and combine with all DMGs results.10x (n=1902) file to keep methylation information to then calculate hyper and hypomethylated genes for topGO analysis
```{r}
sub_meth_table.10x 
```
...
# Run DESeq2 analysis
dds <- DESeq(gdds)
DEGSeq2.results <- results(dds, contrast=c("treatment","enriched","control"))

#save all DEGs
write.csv(DEGSeq2.results, file = "RAnalysis/Output/WGBS/DMGs_all.csv")
