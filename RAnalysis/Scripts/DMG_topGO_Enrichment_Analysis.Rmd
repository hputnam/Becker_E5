...
GFF3 <- read.csv("RAnalysis/Output/rename_structure_gff3.csv")
...
treatmentinfo <- read.csv("RAnalysis/Data/metadata.RNAseq.csv", header = TRUE, sep = ",")
str(treatmentinfo)
head(treatmentinfo)

#DEG results
DEG.res <- read.csv("RAnalysis/Output/DEGs.all.20240530.csv")
#nrow(DEG.res)
...
GOdata.up_BP
numSigGenes(GOdata.up_BP) #7736
sum(feasible(GOdata.up_BP)) #15385
```
...
weight01.fisher.up_BP <- runTest(GOdata.up_BP, statistic = "fisher")

# Generate a results table
allRes_up_BP <- GenTable(GOdata.up_BP, 
                      classicFisher.up = weight01.fisher.up_BP, 
                      ranksOf = "classicFisher.up", 
                      topNodes = length(score(weight01.fisher.up_BP)))

# Convert character columns to numeric
allRes_up_BP$classicFisher.up <- as.numeric(allRes_up_BP$classicFisher.up)
...
GOdata.down_BP
numSigGenes(GOdata.down_BP) #7736
sum(feasible(GOdata.down_BP)) #15385
```
------------------------- topGOdata object -------------------------
...
```{r}
# weight01 is the default algorithm used in Alexa et al. (2006) and Li et al. 2018
weight01.fisher.down_BP <- runTest(GOdata.down_BP, statistic = "fisher")

# Generate a results table
allRes_down_BP <- GenTable(GOdata.down_BP, 
                      classicFisher.down = weight01.fisher.down_BP, 
                      ranksOf = "classicFisher.down", 
                      topNodes = length(score(weight01.fisher.down_BP)))

# Convert character columns to numeric
allRes_down_BP$classicFisher.down <- as.numeric(allRes_down_BP$classicFisher.down)
...
```{r}
# view plot in line first
showSigOfNodes(GOdata.down_BP, score(weight01.fisher.down_BP), firstSigNodes = 5, useInfo = 'all')
...
pdf(pdf_file_path)
# Call printGraph
printGraph(GOdata.down_BP, weight01.fisher.down_BP, firstSigNodes = 10, fn.prefix = "tGO", useInfo = "all", pdfSW = FALSE)
# Close the PDF device
dev.off()
```
...
GOdata.up_MF
numSigGenes(GOdata.up_MF) #1564
sum(feasible(GOdata.up_MF)) #2054
```
------------------------- topGOdata object -------------------------
...
```{r}
# weight01 is the default algorithm used in Alexa et al. (2006) and Li et al. 2018
weight01.fisher.up_MF <- runTest(GOdata.up_MF, statistic = "fisher")

# Generate a results table
allRes_up_MF <- GenTable(GOdata.up_MF, 
                      classicFisher.up = weight01.fisher.up_MF, 
                      ranksOf = "classicFisher.up", 
                      topNodes = length(score(weight01.fisher.up_MF)))

# Convert character columns to numeric
allRes_up_MF$classicFisher.up <- as.numeric(allRes_up_MF$classicFisher.up)
...
```{r}
# view plot in line first
showSigOfNodes(GOdata.up_MF, score(weight01.fisher.up_MF), firstSigNodes = 5, useInfo = 'all')
...
pdf(pdf_file_path)
# Call printGraph
printGraph(GOdata.up_MF, weight01.fisher.up_MF, firstSigNodes = 10, fn.prefix = "tGO", useInfo = "all", pdfSW = FALSE)
# Close the PDF device
dev.off()
```
...
GOdata.down_MF
numSigGenes(GOdata.down_MF) #8165
sum(feasible(GOdata.down_MF)) #16359
```
------------------------- topGOdata object -------------------------
...
```{r}
# weight01 is the default algorithm used in Alexa et al. (2006) and Li et al. 2018
weight01.fisher.down_MF <- runTest(GOdata.down_MF, statistic = "fisher")

# Generate a results table
allRes_down_MF <- GenTable(GOdata.down_MF, 
                      classicFisher.down = weight01.fisher.down_MF, 
                      ranksOf = "classicFisher.down", 
                      topNodes = length(score(weight01.fisher.down_MF)))

# Convert character columns to numeric
allRes_down_MF$classicFisher.down <- as.numeric(allRes_down_MF$classicFisher.down)
...
```{r}
# view plot in line first
showSigOfNodes(GOdata.down_MF, score(weight01.fisher.down_MF), firstSigNodes = 5, useInfo = 'all')
...
pdf(pdf_file_path)
# Call printGraph
printGraph(GOdata.down_MF, weight01.fisher.down_MF, firstSigNodes = 10, fn.prefix = "tGO", useInfo = "all", pdfSW = FALSE)
# Close the PDF device
dev.off()
```
...
filt.down.BP <- allRes_down_BP %>% 
  filter(classicFisher.down < 0.05) %>% 
