...
for (methy_stat_id in methy_stat_list) {
  ...
  go_results <- GO.enrich.all %>%
    filter(methy_stat == methylation_status) %>%
    filter(ontology == "BP") %>%
...
plot_data_BP <- methy_stat_go_results_BP %>%
...
dendro_data_BP$labels <- dendro_data_BP$labels %>%
  left_join(methy_stat_go_results_BP %>% dplyr::select(GO.ID, Term) %>% distinct(), by = c("label" = "GO.ID")) %>%
  mutate(label = Term)
...
#            size = 3, nudge_y = 0.03, color = "blue") +
  ...
  labs(x = NULL, y = NULL, size = "Number of Genes", color = "Methylation Status");dendo.BP.plot
...
#Prepare data for plotting by grouping by GO.ID, methy_stat, ParentTerm, and Term
plot_data_MF <- methy_stat_go_results_MF %>%
  group_by(GO.ID, methy_stat, ParentTerm, Term) %>%
  summarise(numInCat = sum(numInCat)) %>%
  ungroup()
...
dendro_data_MF$labels <- dendro_data_MF$labels %>%
  left_join(methy_stat_go_results_MF %>% dplyr::select(GO.ID, Term) %>% distinct(), by = c("label" = "GO.ID")) %>%
  mutate(label = Term)
...
#            size = 3, nudge_y = 0.03, color = "blue") +
  ...
  labs(x = NULL, y = NULL, size = "Number of Genes", color = "Methylation Status");dendo.MF.plot
