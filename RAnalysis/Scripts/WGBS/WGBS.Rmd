...
MD.10x.original <- readRDS("RAnalysis/Output/WGBS/sample_all_methylated_data_10x.Rdata") #read in info

head(MD.10x.original)
```
...
#results: Kruskal-Wallis chi-squared = 141.29, df = 1, p-value < 2.2e-16, significantly higher percent meth in DMGs in enriched treatment
...
num_hyper_genes <- length(hyper_genes) #198
num_hypo_genes <- length(hypo_genes) #167
...
DMG.volcano <- ggplot(final_df, aes(x = log2FoldChange, y = -log10(padj), color = methylation_status)) +
  ...
  scale_color_manual(values = colors, name = "Methylation Status", 
                     ...
                     labels = c("Hypermethylated (198)", "Hypomethylated (167)", "Not Significant"),
