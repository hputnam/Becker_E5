...
final_df <- final_df %>%
  rename(padj = adj.pval.treatment)

final_df <- final_df %>%
  rename(pvalue = pval.treatment)

# then you can install the dev version of a new package to create volcano plots for DMG data
devtools::install_github("loukesio/ggvolc")
# and load the package before use
library(ggvolc)

ggvolc(final_df, size_var = "log2FoldChange", add_seg = TRUE)
