...
ggsave("RAnalysis/Output/WGBS/per.meth.5x.DMG.pdf")
ggsave("RAnalysis/Output/WGBS/per.meth.5x.DMG.png")

#Sanity check plotting of global DMG by treatment
subdf <- MD.5x #%>%	
  #filter(gene== DMG.sig.annot.5x$gene[1])
means <- aggregate(per.meth ~ treatment, data=subdf, FUN=mean)
ses <- aggregate(per.meth ~ treatment, data=subdf, FUN=std.error)
...
ggplot(means, aes(x=treatment, y=per.meth)) +
  geom_bar(stat="identity", color="black",
    ...
    xlab("Treatment") + #plot x axis label
    ylab("Percent Methylation") + #plot y axis label
    ylim(0,3.5)+ #set y limits to 0 and 100%
    theme_bw()

ggsave("RAnalysis/Output/WGBS/per.meth.5x.global.pdf")
ggsave("RAnalysis/Output/WGBS/per.meth.5x.global.png")
