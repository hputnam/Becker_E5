...
Annot <- read.csv("RAnalysis/Genome/Pver_gene_annotation_file.txt", header=TRUE, sep="\t", na.string="NA", stringsAsFactors = FALSE, skip=0)
...
# Identifying DMG with significant main effect 
DMG.sig <-results.10x
DMG.sig <- DMG.sig[order(DMG.sig$adj.pval.treatment),]
DMG.sig <- DMG.sig[which(DMG.sig$adj.pval.treatment<0.05), ]

# Annotation of DMG under between treatments 
DMG.sig.annot <- left_join(DMG.sig , Annot, by="gene")
#DMG.sig.annot <- DMG.sig.annot[!duplicated(DMG.sig.annot$gene),]
write.table(DMG.sig.annot, 'RAnalysis/Output/WGBS/Treatment_sig_annot.tsv', sep='\t', row.names=FALSE)

#Sanity check plotting of DMGs by treatment
subdf <- MD.10x %>%	
  filter(gene== DMG.sig.annot$gene[4])
...
DMG.trt <- as.character(DMG.sig$gene) #set the enrichment test list
...
gene.vector=as.integer(ALL.vector%in%DMG.trt.vector) #Construct new vector with 1 for DEG and 0 for others
...
GO.sig.trt <- GO.wall %>%
  ...
  arrange(., ontology)
write.csv(GO.sig.trt , file = "RAnalysis/Output/WGBS/GO.sig.trt.csv")
...
MFplot <- GO.MF %>% mutate(term = fct_reorder(term, -over_represented_pvalue)) %>% 
    ...
    geom_tile(aes(fill=over_represented_pvalue, width = 1)) + 
      ...
      element_text(size = 11))
fig5 <- BPplot + MFplot
ggsave("RAnalysis/Output/WGBS/DMG_heatmap_GO_terms.pdf", fig5, width = 20, height = 20, units = c("in"))
...
write.csv(GO.enrichment.summary.table, 'RAnalysis/Output/WGBS/GO.enrichment.summary.table.ontology.csv') 
```
