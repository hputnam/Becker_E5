...
DEG.DMG.meth.express <- ggplot(df_filtered, aes(x=per_meth, y=log_gene,)) + 
    geom_point() + ylim(0, 2.4) + xlim(0,50) +
    geom_smooth(method=lm,colour="orange",size=2,na.rm = TRUE) +
    scale_color_viridis() +
    labs(x="DNA Methylation (%)", y = "log10 (mean expression +1)") +
  stat_cor(method = "kendall", digits = 2.8, label.x = 40, label.y = 0.5 ) +
    theme_cowplot();DEG.DMG.meth.express

### Figure Mean Methylation vs Gene Expression###
ggsave(DEG.DMG.meth.express, filename="RAnalysis/Output/DEG_DMG_correlation.png", height = 10, width = 10)

# 249 DEGs do not have methylation data due to filtering for methylation 

# Select genes that are in both DMG and DEG datasets
common_genes <- intersect(DMG_rename$gene_id, DEG$gene_id)

# View the common genes
common_genes

# Assuming df_filtered is the dataframe you want to filter
df_select <- subset(df_filtered, gene_id %in% common_genes)
