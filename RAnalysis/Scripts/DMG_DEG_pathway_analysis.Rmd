...
#saveRDS(gSum_red,"RAnalysis/Data/Multi_geneSummaryReduced_all.RData")
```
...
#plot of DMG METH EXPRESS, 239 DEGs do not have methylation data due to filtering for methylation so really a DMG correlation
DMG.meth.express <- ggplot(df_filtered, aes(x=per_meth, y=log_gene)) + 
    geom_point() + ylim(0, 2.4) + xlim(0,40) +
    geom_smooth(method=lm,colour="orange",size=2,na.rm = TRUE) +
    scale_color_viridis() +
    labs(x="DNA Methylation (%)", y = "log10 (mean expression +1)") +
  stat_cor(method = "kendall", digits = 2.8, label.x = 35, label.y = 0.5 ) +
...
DEG.meth.express <- ggplot(df_DEG_rename, aes(x=per_meth, y=log_gene)) + 
    geom_point() + ylim(0, 2.4) + xlim(0,20) +
    geom_smooth(method=lm,colour="orange",size=2,na.rm = TRUE) +
    scale_color_viridis() +
    labs(x="DNA Methylation (%)", y = "log10 (mean expression +1)") +
...
# write dataframe
write.csv(gene_list_separated, "RAnalysis/Output/gene_list_separated.csv")
...
# Check how many rows have non-NA values for both log_gene and per_meth
valid_rows <- gene_filtered[!is.na(gene_filtered$log_gene) & !is.na(gene_filtered$per_meth), ]
...
corr.figures <- plot_grid(meth.express.compare, DEG.meth.express, DMG.meth.express, hypoth_compare, labels = c('A', 'B', 'C', 'D'), label_size = 16)
...
# Calculate log2 fold change for the target genes
target_genes_summary$log2_fold_change <- log2(mean_express_E[rownames(target_genes_summary)] / mean_express_C[rownames(target_genes_summary)])

# Handle infinite and NaN values
target_genes_summary$log2_fold_change[is.infinite(target_genes_summary$log2_fold_change) | is.nan(target_genes_summary$log2_fold_change)] <- NA

# Scatterplot of fold change with y-axis intervals and customized text sizes
ggplot(target_genes_summary, aes(x = gene, y = log2_fold_change, color = log2_fold_change)) +
  geom_point(size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 1) +  # Line at fold change = 1
  theme_bw() +
  ...
  #scale_y_continuous(breaks = seq(min(target_genes_summary$log2_fold_change, na.rm = TRUE), 
 #                                 max(target_genes_summary$log2_fold_change, na.rm = TRUE), by = 0.6))  # Set y-axis intervals at 1
...
# Calculate for gene expression data:
# Filter gene_summary for the genes of interest
target_meth_summary <- meth.change[meth.change$gene %in% gene_list_vector,]

# Calculate log2 fold change for the target meth
target_meth_summary$log2_fold_change <- log2(mean_express_E[rownames(target_meth_summary)] / mean_express_C[rownames(target_meth_summary)])

# Handle infinite and NaN values
target_meth_summary$log2_fold_change[is.infinite(target_meth_summary$log2_fold_change) | is.nan(target_meth_summary$log2_fold_change)] <- NA

# Scatterplot of fold change with y-axis intervals and customized text sizes
ggplot(target_meth_summary, aes(x = gene, y = log2_fold_change, color = log2_fold_change)) +
  geom_point(size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 1) +  # Line at fold change = 1
  theme_bw() +
  labs(title = "log2fold Change in Methylation in Genes of Interest Between Enriched and Control Treatments",
