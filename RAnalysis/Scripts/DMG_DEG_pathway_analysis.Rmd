...
## load packages
if (!requireNamespace("ggpmisc", quietly = TRUE)) { BiocManager::install("ggpmisc") }
...
# Run the linear regression model
model <- lm(log_gene ~ per_meth, data = df_DEG_rename)

# Summary of the model to see the results
model_summary <- summary(model)

# Extract coefficients and R-squared
r_squared.DEG <- model_summary$r.squared
...
DEG.meth.express <- ggplot(df_DEG_rename, aes(x=per_meth, y=log_gene)) + 
  ...
  ggtitle("DEGs (significant expression)") +  # Add title
    annotate("text", x = 60, y = 2.0, 
             label = paste("R² =", round(r_squared.DEG, 2)), 
             size = 5, hjust = 0, vjust = 18, color = "blue") +
...
# write dataframe
write.csv(gene_list_cleaned, "RAnalysis/Output/gene_list_cleaned.csv")
...
# Run the linear regression model
model <- lm(log_gene ~ per_meth, data = combined_data)

# Summary of the model to see the results
model_summary <- summary(model)

# Extract coefficients and R-squared
r_squared.interest <- model_summary$r.squared

# Create plot with model results
all_functions <- ggplot(combined_data, aes(x = per_meth, y = log_gene)) + 
    ...
    annotate("text", x = 60, y = 2.0, 
             label = paste("R² =", round(r_squared.interest, 2)), 
             size = 5, hjust = -1, vjust = 16, color = "blue") +
...
```{r}
# Combine DEG and DMG with combined_data
data <- final_df %>%
...
# Count number of genes per function
gene_counts_by_function <- data %>%
  group_by(Function) %>%
  summarise(gene_count = n_distinct(gene_id)) %>%
  arrange(desc(gene_count))

print(gene_counts_by_function)

# Prepare the data
plot_data <- data %>%
  dplyr::select(gene_id, Function, DEG_status, DMG_status) %>%
  distinct() %>%
  mutate(
    DEG_status = case_when(
      DEG_status == "Upregulated" ~ "Up",
      DEG_status == "Downregulated" ~ "Down",
      TRUE ~ "Not in DEG"
    ),
    DMG_status = case_when(
      DMG_status == "Hypermethylated" ~ "Hyper",
      DMG_status == "Hypomethylated" ~ "Hypo",
      TRUE ~ "Not in DMG"
    )
  ) %>%
  pivot_longer(cols = c(DEG_status, DMG_status), 
               names_to = "Category", 
               values_to = "Status")

# Create the plot
ggplot(plot_data, aes(x = Category, y = gene_id, fill = Status)) +
  ...
  scale_fill_manual(values = c(
    ...
    "Hyper" = "darkred", 
    "Hypo" = "darkblue", 
    "Not in DEG" = "lightgrey",
    "Not in DMG" = "lightgrey"
  )) +
  theme_minimal() +
  theme(
  ...
  labs(x = "", y = "", fill = "Status", 
       title = "DEG and DMG Status by Gene and Function")

# Save the plot
ggsave("Bioinformatics/Output/gene_func_plot.png", width = 15, height = 15)

# Assuming your data is in a dataframe called 'gene_data'

# Filter for nutrient metabolism genes and prepare the data
nutrient_data <- data %>%
  filter(Function == "Nutrient metabolism") %>%
  dplyr::select(gene_id, DEG_status, DMG_status) %>%
  distinct() %>%
  mutate(
    DEG_status = case_when(
      DEG_status == "Upregulated" ~ "Up",
...
# Create the plot
ggplot(nutrient_data, aes(x = Category, y = gene_id, fill = Status)) +
  geom_tile(color = "white") +
  scale_fill_manual(values = c(
    "Up" = "red", 
    "Down" = "blue", 
  ...
  labs(x = "", y = "", fill = "Status", 
       title = "DEG and DMG Status for Nutrient Metabolism Genes")

# Save the plot
ggsave("Bioinformatics/Output/gene_func_nutrients.png", width = 15, height = 20)

# Count unique gene_ids
unique_gene_count <- length(unique(rownames(nutrient_data)))

# Print the result
print(paste("Number of unique gene_ids:", unique_gene_count))
