...
for(i in 1:length((combined.data$gene_id))){
  ...
  combined.data1 <- subset(combined.data, gene_id == gs[i], na.rm=TRUE)
  
  fit <- glmer(count ~ treatment + (1|treatment:block), 
...
DEG.sig.results_model <- subset(results, pval.treatment < 0.05)
